...
## Requirements:
 ...
 4) Poly file for your region extract; available from Geofabrik.de website.
...
and 38 minutes away from midnight (we disregard the seconds part here, no change
files for seconds). This is a **gap** in time we need to fill - add to our extract
data file - with change files from planet OSM server.
...
extract data file verifies that the operation was successful.

You can use **`--extend`** option with `osmium fileinfo` for better verification
with timestamps for data.

Now we have two extract data files we can produce a change file from both. Note
here that in this case the change file will not be for standard time interval;
it will be for one day plus 3 hours and 38 minutes.

This new extract data file can be used to initialize an "overpass" database that
is aligned with planet OSM daily update time so this database can use planet change
files for updating.
...
Calling `getdiff` with "BEGIN / --begin" only makes the program look for "end" from
the remote server and now it will append downloaded filenames to **newerFiles.txt**
file not "rangeList.txt". The program gets sequence number for "end" from the
`state.txt` file on the root page for the "SOURCE / --source" key / argument. Before
it exits, the program writes the sequence number for last downloaded file in the
**previous.seq** file in its work directory and with that file presence the "BEGIN
/ --begin" is ignored.
...
The script "mk_regional_osc.sh" is used to make regional change file from planet
change file, script usage is:
```
usage:
...
The "target.name" file is a text file with last extract data filename. This is the
file made in the previous step with the "date" part in its name. The "target.name"
file is created in the "region" directory with command like:
